    ...
    <div v-if="dataSpp.source">
        <b-card no-body class="rounded shadow">
                ...
                <div class="d-sm-block d-md-flex my-2">
                    ...
                    <template v-if="!hideControls && !isNew && !readOnly">
                        ...
                        <spp-controls
                            ...
                            @printSpp="printSpp"
                            @view-pibk="
                                $router.push(`/pibk/${dataSpp.pibk.data.id}`)
                            "
                            @create-pibk="viewCreatePibkDialog = true"
                            @restore-cd="onRestoreCd"
                            @restore-pibk="onRestorePibk"
                        >
                            <div class="d-inline-block my-2">
                                <!-- IP Controls -->
                                <ip-controls
                                    :disabled="
                                        docHasLink(dataSpp, 'pibk') ||
                                        lhpIsLocked(dataSpp)
                                    "
                                    :uri="`/spp/${dataSpp.id}/ip`"
                                    :data="
                                        dataSpp.instruksi_pemeriksaan
                                            ? dataSpp.instruksi_pemeriksaan.data
                                            : null
                                    "
            ...
            <b-tabs card lazy v-model="tabId">
                ...
                <b-tab
                    v-if="dataSpp.instruksi_pemeriksaan && !hideIp"
                    title="Instruksi Pemeriksaan"
                >
                    <b-row>
                        <b-col md="6">
                            <ip-contents
                                :value="dataSpp.instruksi_pemeriksaan.data"
                                disabled
                            />
                        </b-col>
                    </b-row>
                ...
                <b-tab v-if="getLhp(dataSpp, true)" title="LHP">
                    ...
                    <attachment-bucket
                        show
                        disabled
                        :endpoint="lhpAttachmentEndpoint(dataSpp)"
                    />
                </b-tab>
            </b-tabs>
...
export default {
    ...
    computed: {
        ...
        detailUri() {
            let index = "cd";
            if (this.dataSpp.source_type == "App\\PIBK") {
                index = "pibk";
            }

            return `/${index}/${this.dataSpp.source.data.id}/details`;
        },
        ...
        detailSaveUri() {
            ...
            return `/${index}/${this.dataSpp.source.data.id}/penetapan`;
        },
    ...
    methods: {
        ...
        async onRestoreCd() {
            var result = await this.$bvModal.msgBoxConfirm(
                `Selesaikan Customs Declaration atas SPP ini?`,
                {
            ...
            if (result) {
                ...
                this.api
                    .deleteSpp(this.dataSpp.id)
                    .then((e) => {
                        this.setBusyState(false);
                        // redirect @next tick
                        this.$nextTick(() => {
                            this.$router.replace(
                                `/cd/${this.dataSpp.source.data.id}`
                            );
                        });
                    })
                    .catch((e) => {
        ...
        // when restore pibk clicked
        async onRestorePibk() {
            var result = await this.$bvModal.msgBoxConfirm(
                `Selesaikan PIBK atas SPP ini?`,
                {
                    title: `Restore data PIBK`,
                    size: "md",
                    buttonSize: "md",
                    okVariant: "danger",
                    okTitle: "YES",
            ...
            if (result) {
                ...
                this.api
                    ...
                    .then((e) => {
                        ...
                        this.$nextTick(() => {
                            this.$router.replace(
                                `/pibk/${this.dataSpp.source.data.id}`
                            );
                        });
                    })
                    .catch((e) => {
        ...
        onCreatePibk(data) {
            ...
            // call it
            this.api
                .putEndpoint(`/spp/${this.dataSpp.id}/pibk`, data)
                .then((e) => {
                    this.setBusyState(false);
