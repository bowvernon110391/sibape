        ...
        <b-row>
            ...
            <b-col md="6">
                <b-form-group label="Alamat" label-for="alamat">
                    <b-form-textarea :disabled="disableInput">
                    </b-form-textarea>
                </b-form-group>
            </b-col>
        </b-row>
           ...
           if (!this.dataBpj.id) {
              this.api.createBpj(this.dataBpj)
               .then(e => {
                  vm.setBusyState(false)
                  var newId = e.data.id
                  vm.showToast('Data SPP Tersimpan!', `Data Bpj tersimpan dengan id #${newId}`, "success")
                  // re route
                  this.$router.replace({
                     name: 'ViewSpp',
                     params: {
              ...
              this.api.updateBpj(this.id, this.dataBpj)
               .then(e => {
                  vm.setBusyState(false)
                  vm.showToast('Data SPP terupdate!', `Berhasil mengupdate data BPJ #${vm.id}`, "success")
                  // reload biar sinkron?
